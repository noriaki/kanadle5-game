 ...
 */
export async function getDailyWordRedis(date: Date, wordMaster: WordMaster): Promise<string> {
  try {
    ...
    const cachedWordId = await safeRedisOperation(() => redis.get(cacheKey), null);
    ...
    if (selectedWordEntity) {
      ...
      await safeRedisOperation(() => redis.set(cacheKey, selectedWordEntity.id), null);
    }
 ...
 */
async function generateDailyWord(date: Date, wordMaster: WordMaster): Promise<string> {
  try {
